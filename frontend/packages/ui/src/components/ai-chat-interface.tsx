...
// Define UserRole locally since we can't import from apps/web
type UserRole = "patient" | "counselor" | "admin" | "guest"
  ...
  return (
    <div className={`fixed inset-0 z-50 flex h-screen bg-background ${className}`}>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        {/* AI Chat Content */}
        <motion.div 
          className="flex-1 flex overflow-hidden mt-4 mx-4 mb-4 rounded-lg border border-border bg-background"
        ...
        >
          {/* Chat Sidebar */}
          <motion.div 
            className="w-80 bg-gradient-to-br from-primary/5 via-background to-primary/10 border-r border-border flex flex-col rounded-l-lg"
          ...
          {/* Main Chat Area */}
          <motion.div 
            className="flex-1 flex flex-col items-center justify-center p-12 bg-background border-l border-border/50 rounded-r-lg"
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
